    ...
    public function verify($mobile, $code, $type)
    {
        try {
            ...
            if (!$this->checkMsgType($type)) {
                throw new \Exception('Código de verificação incorreto');//非法的型验证码类型
            }
            ...
            if (empty($data) || $data['expire_time'] < time()) {
                throw new \Exception('Código de verificação incorreto');//验证码有误
            }
    ...
    public function send($mobile, $type, $params = '')
    {
        try {
            ...
            curl_setopt_array($curl, array(
              ...
              CURLOPT_POSTFIELDS => 'appkey=' .$appkey  .  '&secretkey=' . $appcode .'&phone='. '55' .$mobile.'&content='. urlencode($message),

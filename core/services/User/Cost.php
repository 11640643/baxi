...
class Cost extends Service
{
    ...
    public function cash2($orderNo,$uid, $money, $bankId, $bank_name,$bank_card, $json = '')
    {
        try {
            $lockKey = "uid:$uid:cash";
            if (!$this->di['s_user']->lock($lockKey, 5)) {
            ...
            if (!$s_funds = $this->di['s_funds']->add($uid, -$money, 'money', 'cost', $this->translate['blance_cost'], $cid, 'Y', $moneyArray[0], $moneyArray[1])) {
                throw new \Exception($this->translate['new_lang_get_cash_error']);
            }
            $this->di['db']->commit();
            return true;
